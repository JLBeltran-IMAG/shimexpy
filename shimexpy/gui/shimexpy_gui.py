...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.save_result_btn.setToolTip("Save the processed result as an image")
    ...
    def _save_result(self):
        """Guardar el resultado procesado como imagen."""
        if self.contrast_result is None:
            self._show_error("No hay resultado para guardar. Primero procese las imágenes.")
            return

        # Obtener el tipo de contraste actual
        contrast_type = self.contrast_type_combo.currentText()
        
        # Obtener el array correspondiente
        if contrast_type == "all":
            # Si está seleccionado "all", guardamos el contraste que se está mostrando actualmente
            if "absorption" in self.contrast_result:
                result_array = self.contrast_result["absorption"]
                current_type = "absorption"
            else:
                self._show_error("No se encontró un resultado válido para guardar.")
                return
        else:
            # Para cualquier otro tipo, usamos el contraste seleccionado
            if contrast_type in self.contrast_result:
                result_array = self.contrast_result[contrast_type]
                current_type = contrast_type
            else:
                self._show_error(f"No se encontró el resultado de {contrast_type} para guardar.")
                return
        ...
        if file_path:
            try:
                # Asegurarnos de que el array es 2D para guardarlo como imagen
                if result_array.ndim > 2:
                    # Si es un array 3D, guardamos la primera componente
                    result_array = result_array[0]

                # Usar la función save_image del módulo shimexpy.io
                save_image(result_array, file_path)
            except Exception as e:
                self._show_error(f"Error al guardar el resultado: {str(e)}")
